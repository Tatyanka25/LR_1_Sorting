...
void insertion_sort(T* arr, int size) {  
	for (int i = 1; i < size; ++i) {  // Цикл для прохода по элементам массива arr, начиная со второго элемента
		T key = arr[i];  // Сохранение текущего элемента массива в переменную key
		int j = i - 1;  // Инициализация переменной j со значением i - 1
		while (j >= 0 && arr[j] > key) {  // Цикл while, продолжающийся, пока не достигнут начало массива и текущий элемент больше key
			arr[j + 1] = arr[j];  // Сдвиг всех больших элементов вправо
			j = j - 1;  // Уменьшение значения j
		}
		arr[j + 1] = key;  // Вставка key в правильную позицию в отсортированной части массива
	}
}
